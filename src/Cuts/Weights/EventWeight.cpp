...
Bool_t EventWeight::Apply()
{
 ...
 if(isPileUpWgt()) {
   if(_reCalPU){
       ...
       if ( _mcPV->GetBinContent(PVbin) > 0){
...
std::tuple<Double_t,Double_t,Double_t> EventWeight::getChargeMisWeight(EventContainer* EventContainerObj){
  ...
  if(fabs(lep1.pdgId())==11){
    //Get the bin 
    int xAxisBin1  = std::max(1, std::min(_chargeMis->GetNbinsX(), _chargeMis->GetXaxis()->FindBin(lep1.Pt())));
  ...
  if(fabs(lep2.pdgId())==11){
    int xAxisBin2  = std::max(1, std::min(_chargeMis->GetNbinsX(), _chargeMis->GetXaxis()->FindBin(lep2.Pt())));
...
std::tuple<Double_t,Double_t,Double_t> EventWeight::getTriggerWeight(EventContainer * EventContainerObj){
  ...
  if(dataEra==2016){
      if(_whichTrigger>=2 && _whichTrigger <=5){
        if(category ==3){//mm
            TriggerWeight = 1.010; 
            TriggerWeightUp = 1.010 + 0.01;
            TriggerWeightDown = 1.010 - 0.01;
        }else if (category ==1){//ee
            TriggerWeight = 1.020; 
            TriggerWeightUp = 1.020 + 0.02;
            TriggerWeightDown = 1.020 - 0.02;
        }else if (category ==2){//em
            TriggerWeight = 1.020; 
            TriggerWeightUp = 1.020 + 0.01;
            TriggerWeightDown = 1.020 - 0.01;
        }
      }else if(_whichTrigger==6 || _whichTrigger==7){//4l
          TriggerWeight = 1.00; 
          TriggerWeightUp = 1.06;
          TriggerWeightDown = 0.94;
      }
...
Double_t EventWeight::getBTagReshape(EventContainer * EventContainerObj, std::string syst){
  ...
  int eventNumber = EventContainerObj->eventNumber;
  int runNumber = EventContainerObj->runNumber;
  int lumiBlock = EventContainerObj->lumiBlock;
  Bool_t debug = false;
  /*
  if (lumiBlock ==1 && (eventNumber == 3619 || eventNumber==830 || eventNumber == 1458 || eventNumber==3526) && syst=="central"){
      debug = true;
  ...
  */
  for (auto const & jet : EventContainerObj->jets){
    if (jet.GethadronFlavour() == 5){
        ...
        if (debug){std::cout<<"jet pt: "<<jet.Pt()<<" jet eta: "<< jet.Eta()<<", Flavour is "<< jet.GethadronFlavour() << ", read Systematic "<<syst<<", returned value <bTagWeight, jetSF>: <"<<bTagWeight <<", "<<jetSF<<">";}
        if (jetSF == 0) jetSF = _bTagCalibReader.eval_auto_bounds("central", BTagEntry::FLAV_B, fabs(jet.Eta()), jet.Pt(), jet.bDiscriminator());
        bTagWeight *= jetSF;
        if (debug){std::cout<< ", <bTagWeight, jetSF> after jetSF==0 check : <"<<bTagWeight <<", "<<jetSF<<">"<<std::endl;}
